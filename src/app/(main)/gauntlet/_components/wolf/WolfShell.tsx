...
function computeMaxIndex(state: WolfState | null, isGameOver: boolean): number {
    if (!state) return 0;
...
export function WolfShell({ onResetToSetup }: Props) {
    const {
        state,
        submitHole,
        editHole,
    ...
    // Wolf rotation + per-player points are deterministic from holes; we don't
    // need a "playersEntering" derivation for live entry (there's no chase
    // state). We do need it for displaying past holes consistently.
    ...
    const handleSubmit = (hole: WolfHole) => {
        submitHole(hole);
        setViewedHole(clampedViewedHole + 1);
    };

    const handleSaveEdit = (holeIndex: number, hole: WolfHole) => {
