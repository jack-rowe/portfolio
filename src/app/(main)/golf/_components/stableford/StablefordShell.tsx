...
export function StablefordShell({ onResetToSetup }: Props) {
    const {
        state,
        submitHole,
        editHole,
    ...
    const handleSubmit = (hole: StablefordHole) => {
        submitHole(hole);
        setViewedHole(clampedViewedHole + 1);
    };

    const handleSaveEdit = (holeIndex: number, hole: StablefordHole) => {
