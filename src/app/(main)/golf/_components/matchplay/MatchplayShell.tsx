...
export function MatchplayShell({ onResetToSetup }: Props) {
    const {
        state,
        submitHole,
        editHole,
    ...
    const handleSubmit = (hole: MatchplayHole) => {
        submitHole(hole);
        setViewedHole(clampedViewedHole + 1);
    };

    const handleSaveEdit = (holeIndex: number, hole: MatchplayHole) => {
        ...
        >
            ...
            <EndRoundDialog
                ...
                onConfirm={() => {
                    ...
                    toast.success(
                        `Match ended after hole ${String(state.holes.length)}`,
                    );
                }}
            />
