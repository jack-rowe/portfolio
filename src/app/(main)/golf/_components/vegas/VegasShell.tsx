...
import { Dice5, Flag, RotateCcw } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { useVegas } from "../../_hooks/use-vegas";
import type { VegasHole, VegasState } from "../../_lib/vegas/types";
...
function computeMaxIndex(state: VegasState | null, isGameOver: boolean): number {
    if (!state) return 0;
...
export function VegasShell({ onResetToSetup }: Props) {
    const {
        state,
        submitHole,
        editHole,
    ...
    const handleSubmit = (hole: VegasHole) => {
        submitHole(hole);
        setViewedHole(clampedViewedHole + 1);
    };

    const handleSaveEdit = (holeIndex: number, hole: VegasHole) => {
        ...
        >
            <header className="sticky top-0 z-10 bg-background/90 backdrop-blur border-b border-border py-3 mb-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <Dice5 aria-hidden="true" className="w-5 h-5 text-primary" />
